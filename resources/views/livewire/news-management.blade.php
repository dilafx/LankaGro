                ...
                @forelse ($newsItems as $item)
                    <tr wire:key="{{ $item->id }}">
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                            {{ Str::limit($item->title, 50) }}</td> {{-- Limit title length --}}
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                            {{ $item->user->name ?? 'N/A' }}</td> {{-- Show author name --}}
                        <td class="px-6 py-4 whitespace-nowrap">
                            {{-- Status Badge --}}
                            <span
                                class="inline-flex px-2 py-1 text-xs rounded-full {{ $item->status == 'published' ? 'bg-green-100 dark:bg-green-700 text-white dark:text-green-100' : 'bg-yellow-100 dark:bg-yellow-700 text-white dark:text-yellow-100' }}">
                                {{ ucfirst($item->status) }}
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            ...
                            <button wire:click="delete({{ $item->id }})"
                                wire:confirm="Are you sure you want to delete this news article?"
                ...
                <div class="mt-3">
                    ...
                    <form wire:submit.prevent="save">
                        <div class="space-y-4">
                            ...
                            {{-- Image Upload --}}
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Featured
                                    Image</label>
                                {{-- Preview Existing Image --}}
                                @if ($isEditing && $existingImageUrl && !$image)
                                    <img src="{{ $existingImageUrl }}" alt="Current Image"
                                        class="h-20 w-auto rounded my-2 border border-gray-300 dark:border-gray-600">
                                    <p class="text-xs text-gray-500 dark:text-gray-400">Current image. Upload a new file
                                        to replace it.</p>
                                @endif
                                {{-- Preview New Image --}}
                                @if ($image)
                                    <img src="{{ $image->temporaryUrl() }}" alt="Image Preview"
                                        class="h-20 w-auto rounded my-2 border border-gray-300 dark:border-gray-600">
                                @endif
                                <input type="file" wire:model="image" id="image-{{ $this->getId() }}"
                                ...
                                <div wire:loading wire:target="image"
                                    class="text-sm text-gray-500 dark:text-gray-400 mt-1">Uploading...</div>
                                @error('image')
                                    <span class="text-red-500 text-sm">{{ $message }}</span>
                                @enderror
                            </div>
